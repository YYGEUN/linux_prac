...
# HOST = '192.168.15.158' # 폰
HOST = '192.168.50.31' # loopback # 교육장
PORT = portnum
client_socket = None
...
class Form(QtWidgets.QMainWindow):
    ...
    def set_fuel_bar(self, signo):
        ...
        if signo == "charge":
            ...
            self.ui.fuelmsg.setText("주유 완료")
            self.ui.fuelmsg.setStyleSheet("Color : black")
            self.msg.setText("-")
        else:
            if int(signo) <= 10:
                self.fuelmsg.setText("연료 부족")
                self.fuelmsg.setStyleSheet("Color : red")
                if int(signo) <= 1:
                    self.ui.fuelmsg.setText("운행 불가")
                    self.ui.fuelmsg.setStyleSheet("Color : red")
                    self.ui.system.setChecked(False)
                    self.msg.setText("연료가 부족하여 종료합니다")
                    self.ui.fuelmsg.setStyleSheet("Color : red")
                    self.ui.accel.setEnabled(False)
                    self.ui.brake.setEnabled(False)
                    self.ui.lh.setEnabled(False)
